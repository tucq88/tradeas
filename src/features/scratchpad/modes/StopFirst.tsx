...
export function StopFirst({ side, balance, riskPct }: ScratchpadCtx) {
  ...
  const out = preview.errors.length === 0 ? computeStopFirst({
    side,
      ...
      {out && (
        <>
          <div className="border-t border-border-1 pt-2">
            <MetricRow label="max safe size" value={fmtUSD(out.notional)} />
            <MetricRow label="qty" value={fmtNum(out.qty, 4)} />
            <MetricRow label="margin needed" value={fmtUSD(out.margin)} />
          </div>
          <div className="border-t border-border-1 pt-2">
            <MetricRow
              label="1R target"
